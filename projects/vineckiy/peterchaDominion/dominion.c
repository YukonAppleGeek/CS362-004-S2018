...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card ) 
    {
    case adventurer:
		return adventurerCard(card, state);
			
    case council_room:
    	return councilRoomCard(card, state, handPos);
    ...
    case remodel:
	  return remodelCard(card, choice1, choice2, choice3, state, handPos);

    case smithy:
	  return smithyCard(card, state, handPos);
    
	case village:
	  	return villageCard(card, state, handPos);
     /* 
	  //+1 Card
      drawCard(currentPlayer, state);
